...
public class StartTaskCommand extends Command {
    TodoService todoService = new TodoService();
    private static final Logger LOG = Logger.getLogger(StartTaskCommand.class);

    public StartTaskCommand() throws DBException {
    }
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, AppException {
        ...
        if(taskId!=null && !taskId.isEmpty()){
            try {
                todoService.startTodo(Long.parseLong(taskId));
                LOG.debug("Started TODO id = " + taskId);
            } catch (DBException e) {
                LOG.error("{}", e);
